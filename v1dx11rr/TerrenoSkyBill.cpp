            ...
            if (!dxrr->Parado) {
                ...
                if (gamePad->IsConnected())
                {
                    ...
                    if (grados > 0.19 || grados < -0.19) {
                        dxrr->izqder = grados / 30;
                        dxrr->CarroAngulo = grados * 100 * 3.1416 / 180 * .25f;
                    }
                    else if (grados == 0) {
                        dxrr->CarroAngulo = 0;
                    }
                    ...
                    if (grados > 0.19 || grados < -0.19)dxrr->arriaba = grados / 45;


                    float velocidad = (float)gamePad->GetState().Gamepad.sThumbLY / 32767.0 * 2;
                    if (velocidad > 0.19 || velocidad < -0.19) {
                        if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
                            velocidad *= 24; //Nitro
                        else if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) velocidad *= 8; //cámara lenta
                        else velocidad *= 15; //Velocidad base
