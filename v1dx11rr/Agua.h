...
private:
	...
	ID3D11ShaderResourceView* colorMap, *colorMap2;
	...
	ID3D11Buffer* FactorCB; //Copia
	float Factor; //Copia

public:
	Agua(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* colorTexturePath2, WCHAR* specularTexturePath, float _posX, float _posZ)
	{
		...
		CargaParametros(ModelPath, colorTexturePath, colorTexturePath2, specularTexturePath);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* diffuseTex2, WCHAR* specularTex)
	{
		HRESULT d3dResult;
		...
		bool compileResult = CompileD3DShader(L"Agua.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &FactorCB); //Copia
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool Cam, int cam, float inputEje)
	{
		...
		angulo += .002f;  //Se divide entre el número de modelos que usen la función draw wn dxrr
		...
		d3dContext->UpdateSubresource(FactorCB, 0, 0, &Factor, 0, 0); //Copia
		...
		d3dContext->PSSetConstantBuffers(5, 1, &FactorCB); //Copia
