...
private:
	...
	ID3D11ShaderResourceView* bumpMap; //Copia
	...
	ID3D11Buffer* FactorCB;
	float Factor;

public:
	ModeloRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath,
		WCHAR* colorTexturePath, WCHAR* specularTexturePath, WCHAR* bumpTexturePath, float _posX, float _posZ)
	{
		...
		CargaParametros(ModelPath, colorTexturePath, specularTexturePath, bumpTexturePath);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	//Copia las variables con "bump"
	~ModeloRR()
	{
		//libera recursos
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex, WCHAR* bumpTex) //<- //Copia
	{
		HRESULT d3dResult;
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, bumpTex, 0, 0, &bumpMap, 0); //Copia
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &specIntensaCB);

		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &FactorCB);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specIntensa, float rot, char angle, float scale, bool Cam, int cam, float inputEje)
	{
		...
		d3dContext->PSSetShaderResources(2, 1, &bumpMap); //Copia

		d3dContext->PSSetSamplers(0, 1, &colorMapSampler);

		//mueve la camara
		D3DXMATRIX TraslacionCam;
		if (cam == 1) {
			D3DXMatrixTranslation(&TraslacionCam, -1, 0, 22);
		}
		else{
			D3DXMatrixTranslation(&TraslacionCam, -1, 0, 35);
		}

		D3DXMATRIX rotationMat;
		D3DXMatrixRotationYawPitchRoll(&rotationMat, 0.0f, 0.0f, 0.0f);

		D3DXMATRIX rotationMat2;
		D3DXMatrixRotationYawPitchRoll(&rotationMat2, 0.0f, 0.0f, 0.0f);

		D3DXMatrixRotationY(&rotationMat2, inputEje);
		...
		if (Cam) {
			if (cam == 3) {
				worldMat = scaleMat * rotationMat2 * TraslacionCam * rotationMat * translationMat;
			}
			else
			{
				worldMat = scaleMat * TraslacionCam * rotationMat * translationMat;
			}
		}
		else {
		...
		angulo += (.002f / 12);  //Se divide entre el número de modelos que usen la función draw wn dxrr

		Factor = (sin(angulo) + 1) / 2;
		...
		d3dContext->UpdateSubresource(specIntensaCB, 0, 0, &specIntensa, 0, 0);
		d3dContext->UpdateSubresource(FactorCB, 0, 0, &Factor, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(4, 1, &specIntensaCB);
		d3dContext->PSSetConstantBuffers(5, 1, &FactorCB);
