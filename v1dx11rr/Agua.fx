...
cbuffer cbChangesOccasionally : register(b5) //Copia toda la estructura
{
    float Factor;
};
struct VS_Input
...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
    ...
    textureColor2 = colorMap2.Sample(colorSampler, pix.tex0);
    textureColor = lerp(textureColor, textureColor2, Factor);
	...
	if (lightIntensity > 0) {
        ...
        color += (float4(1.0f, 1.f, 1.f, 1.0f) /*diffuse color*/ * lightIntensity * (Factor + .2f)); //Copia depués de lightIntensity
